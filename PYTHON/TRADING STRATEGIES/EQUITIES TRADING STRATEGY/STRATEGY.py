...
def STRATEGY(data, symbol_extra, SIGNAL_CHOSEN):
    df = symbol_extra
    ...
    def MACD_SIGNAL_STRATEGY(data, df):
        ...
        buy_price, sell_price, macd_signal = implement_macd_strategy(df['close'], googl_macd)
    ...
    def MACD__STOCH_STRATEGY_SIGNAL(data, df):
        ...
        def POSITION_LOGIC(stoch_macd_signal, df):
            ...
            if len(df) == 0:
                strategy = pd.DataFrame(columns=['close', '%k', '%d', 'macd', 'macd_signal', 'stoch_macd_signal', 'stoch_macd_position'])
                
                for u in range(0, 4):
                    strategy = strategy.append({'close': 0,
                                                '%k': 0,
                                                '%d': 0,
                                                'macd': 0,
                                                'macd_signal': 0,
        ...
        df['%k'], df['%d']                                                                = get_stoch_osc(df['high'], df['low'], df['close'], 14, 3)
        df['macd'], df['macd_signal'], df['macd_hist']                                    = get_macd(df['close'], 26, 12, 9)
        df                                                                                = df.dropna()
        ...
        for o in range(len(googl_ret)):
            list_value.append(googl_ret.iat[o, 0])
